...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const expense = await queryOne(`SELECT e.*, u.name as paid_by_name FROM expenses e
       JOIN users u ON e.paid_by_user_id = u.id WHERE e.id = ?`, [Number(id)]) as any;
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { amount, description, date, paidByUserId, splitMode, splits } = await req.json();
    ...
    await execute("UPDATE expenses SET amount = ?, description = ?, date = ?, paid_by_user_id = ?, split_mode = ? WHERE id = ?", [Number(amount), description, date, Number(paidByUserId), splitMode, Number(id)]);
